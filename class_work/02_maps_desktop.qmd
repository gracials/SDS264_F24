...
- Similarities: you build the graph in layers. 
- Differences: In ggplot us use aes() and leaflet doesn't have this, but you can control the aesthetics thorugh addCircleMarkers(). In ggplotyou have to use plus sign but with leaflet you can continue to use the pipe. In leaflet, you don't use an equal sign, you use the tilde. 
...
polisci_static <- us_states |>
  left_join(state_data, by = c("region" = "state_name"))
print(polisci_static, n = 5, width = Inf)

polisci_interact <- states |>
  mutate(name = str_to_lower(name)) |>
  left_join(state_data, by = c("name" = "state_name")) |>
  print(polisci_interact, n = 5, width = Inf)
class(polisci_interact)
```
...
```{r}
  ...
  ggplot() +
    geom_sf(aes(fill = gun_murder10, colour = "white", linetype = 2))
```
