...
potter_tidy |>
  ...
  filter(sentiment == c("fear", "trust")) |>
  count(sentiment, word, sort = TRUE) |>
  group_by(sentiment) |> #I'm not sure if you mean the 10 ten words associated with fear and then the top 10 associated with trust, or if you mean the top 10 associated with either. If you mean the latter, I would just take this line of code out!
  slice_max(n, n = 10, with_ties = FALSE) |>
  print(n = Inf)
```
...
potter_tidy |>
  ...
  ggplot(aes(x = chapter, y = sentiment, fill = title, color = title)) +
...
hp_bigrams_united <- hp_twowords |>
  unite(bigram, word1, word2, sep = " ")

hp_bigrams_united 

hp_bigram_tf_idf <- hp_bigrams_united |> 
  count(title, bigram) |>
  bind_tf_idf(bigram, title, n) |>
  arrange(desc(tf_idf)) 

hp_bigram_tf_idf |> arrange(desc(tf_idf))
...
hp_twowords2 <- potter_untidy |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
  filter(bigram != "NA")

hp_bigrams_separated <- hp_twowords2 |>
  separate(bigram, c("word1", "word2"), sep = " ") |>
  count(word1, word2, sort = TRUE) |>
  filter(!is.na(word1) & !is.na(word2)) 

hp_negation_words <- c("neither", "nor", "not", "never")

hp_negated_words <- hp_bigrams_separated |>
  filter(word1 %in% hp_negation_words) |>
  inner_join(afinn, by = c(word2 = "word")) |>
  arrange(desc(n))
...
potter_section_words <- potter_tidy |>
  ...
  filter(!word %in% stop_words$word,
         ...
         !word %in% potter_spells$spell_name,
...
potter_word_cors <- potter_section_words |>
  group_by(word) |>
  filter(n() >= 10) |>
  pairwise_cor(word, section, sort = TRUE)
...
potter_word_cors |>
  filter(item1 %in% c("clever", "lived", "time", "fear")) |>
  group_by(item1) |>
  slice_max(correlation, n = 6) |>
  ungroup() |>
  mutate(item2 = reorder(item2, correlation)) |>
...
potter_book_count <- potter_tidy |>
  count(word, title, sort = TRUE)

potter_tidy_dtm <- potter_book_count |>
  filter(!word %in% stop_words$word,
         !is.na(word),
         !word %in% potter_names$firstname,
         !word %in% potter_names$lastname,
...
library(topicmodels)
potter_tidy_lda <- LDA(potter_tidy_dtm, k = 2, control = list(seed = 2003))
potter_tidy_lda


potter_books_topics <- tidy(potter_tidy_lda, matrix = "beta")
three_books_topics
...
A.) A plot to illustrate words with greatest difference between two topics, using log ratio.
```{r}
potter_beta <- potter_books_topics |>
  mutate(topic = paste0("topic", topic)) |>
  pivot_wider(names_from = topic, values_from = beta) |> 
  filter(topic1 > .001 | topic2 > .001) |>
  mutate(log_ratio = log2(topic2 / topic1))
...
potter_beta |>
  ...
  ggplot(aes(log_ratio, term, fill = log_ratio > 0)) +
    geom_col(show.legend = FALSE) +
    labs(x = "Log ratio of Beta values",
         y = "Words in the Harry Potter Series")
```

B.) A table with the gamma variable for each document and topic.
```{r}
potter_documents <- tidy(potter_tidy_lda, matrix = "gamma")
potter_documents
```


- Looking at the gamma variables, all of the books are pretty evenly split between topic 1 and topic 2, which isn't too surprising given that the books seem pretty similar content-wise. Looking at the words with the greatest different between the two topics a possibility for what the two topics represent is that one topic represents anticipation and success and the other represents challenges. 
