...
- The data and weights are unequal length, so R is not calculating it like we want. 
...
- The `call.` option tells you in the error message where in the code the error is coming from. 
...
- x is a vector so there is the possibility to have a list of values where some are na and some aren't. If at least one of the values is NA, taking the sum of x will result in NA and appropriately prompt the message that the vector x cannot include NAs. 
